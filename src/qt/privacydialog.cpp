...
#include "zbamboocontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBOO ought to be enough for anybody." - Bill Gates, 2017
    ui->zBOOpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBOOSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBOO->setEnabled(false);
        ui->pushButtonMintzBOO->setToolTip(tr("zBOO is currently disabled due to maintenance."));

        ui->pushButtonSpendzBOO->setEnabled(false);
        ui->pushButtonSpendzBOO->setToolTip(tr("zBOO is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBOOpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBOO_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBOO is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBOO...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBOO in ") +
...
void PrivacyDialog::on_pushButtonSpendzBOO_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBOO is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBOO
        sendzBOO();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBOO
    sendzBOO();
}

void PrivacyDialog::on_pushButtonZBambooControl_clicked()
{
    ZBambooControlDialog* zBambooControl = new ZBambooControlDialog(this);
    zBambooControl->setModel(walletModel);
    zBambooControl->exec();
}

void PrivacyDialog::setZBambooControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBambooSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBOO()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Bamboo Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBOOpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBOO is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BOO </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBOO</b>";
    ...
    // use mints from zBamboo selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBambooControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBambooControlDialog::GetSelectedMints();
    }
    ...
    // Clear zbamboo selector in case it was used
    ZBambooControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBamboo Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Bamboo, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBamboo Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBOO </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBOO "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBOO "));
    ui->labelzBOOAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBOOSyncStatus->setVisible(fShow);
}
