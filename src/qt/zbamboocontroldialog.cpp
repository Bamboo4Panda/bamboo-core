...
#include "ui_zbamboocontroldialog.h"
...
std::list<std::string> ZBambooControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBambooControlDialog::listMints;

ZBambooControlDialog::ZBambooControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBambooControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBambooControlDialog::~ZBambooControlDialog()
{
    delete ui;
}

void ZBambooControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBambooControlDialog::updateList()
{
...
void ZBambooControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBambooControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBamboo_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBambooControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBambooControlDialog::GetSelectedMints()
{
...
void ZBambooControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
