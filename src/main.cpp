...
#if defined(NDEBUG)
#error "Bamboo cannot be compiled without assertions."
#endif
...

    {
        ...
        if (tx.IsZerocoinSpend()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zBamboo spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zBamboo spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zBamboo spend with serial %s from tx %s is not in valid range\n",
...
CAmount GetSeeSaw(const CAmount& blockValue, int nMasternodeCount, int nHeight)
{
    ...
    int64_t nMoneySupply = chainActive.Tip()->nMoneySupply;
    int64_t mNodeCoins = nMasternodeCount * 50000 * COIN; //25k coins required
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight == 0) {
        ret = blockValue * 0;
    } else if (nHeight <= 31679 && nHeight > 200) {
        ret = blockValue / 10 * 3; //30%
    } else if (nHeight <= 74879 && nHeight > 31679) {
        ret = blockValue / 10 * 3.5; //35%
    } else if (nHeight <= 206279 && nHeight > 74879) {
        ret = blockValue / 10 * 4; //40%
    } else if (nHeight <= 9980044 && nHeight > 206279) {
...
//Revive blocks start from 25 and then each block after
int nStartReviveBlock = 999999998;
int nReviveBlockStep = 999999998;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Bamboo
...
{
    RenameThread("bamboo-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZBOOMinted()
{
...
void RecalculateZBOOSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zBOO supply
...
bool RecalculateBOOSupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if (!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zBamboo with serial %s is already in the block %d\n",
    ...
    //    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zBambooSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert BOO to zBOO
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zBamboo spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zBamboo serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zBamboo mints and %d zBamboo spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Bamboo: We use certain sporks during IBD, so check to see if they are
        // available. If not, ask the first peer connected for them.
        if (!pSporkDB->SporkExists(SPORK_14_NEW_PROTOCOL_ENFORCEMENT) &&
            !pSporkDB->SporkExists(SPORK_15_NEW_PROTOCOL_ENFORCEMENT_2) &&
            !pSporkDB->SporkExists(SPORK_18_REVIVE_PAYMENT_ENFORCEMENT_DEFAULT) &&
