...
RUN deps="alpine-sdk curl autoconf automake libtool boost-dev openssl-dev libevent-dev git" && \
    ...
    mkdir /wallet &&\
    cd /wallet && \
    git clone https://github.com/CryptoCashBack-Hub/BOO.git . &&\   
    ./autogen.sh && \
    ./configure LDFLAGS=-L/opt/db/lib CPPFLAGS=-I/opt/db/include \
      && \
    make install && \
    strip /usr/local/bin/bambood &&\
    strip /usr/local/bin/bamboo-cli &&\
    rm /usr/local/bin/bamboo-tx &&\
    rm /usr/local/bin/test_bamboo &&\
    adduser -D wallet && \
    apk del $deps && \
    rm -r /opt/db/docs /var/cache/apk/* /wallet /db-$DB_VERSION

VOLUME ["/home/wallet/.bamboo"]

EXPOSE 8000/tcp

USER wallet
CMD bambood -printtoconsole
